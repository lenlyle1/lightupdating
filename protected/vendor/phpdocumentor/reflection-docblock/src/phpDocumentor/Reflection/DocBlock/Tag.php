...
class Tag implements \Reflector
{
    ...
    final public static function createInstance(
        ...
        $handler = __CLASS__;
        if (isset($this->$tagHandlerMappings[$matches[1]])) {
            $handler = $this->$tagHandlerMappings[$matches[1]];
    ...
    final public static function registerTagHandler($tag, $handler)
    {
        ...
        if (null === $handler) {
            unset($this->$tagHandlerMappings[$tag]);
            return true;
        }
    ...
    {
        if (!preg_match('/^' . $this->REGEX_TAGNAME . '$/u', $name)) {
            throw new \InvalidArgumentException(
