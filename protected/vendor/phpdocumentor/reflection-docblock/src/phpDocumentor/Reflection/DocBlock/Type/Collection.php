...
class Collection extends \ArrayObject
{
    ...
    public function add($type)
    {
        ...
        $type_parts = explode($this->OPERATOR_OR, $type);
     ...
     *     {@link $this->OPERATOR_OR}.
     */
    public function __toString()
    {
        return implode($this->OPERATOR_OR, $this->getArrayCopy());
    }
    ...
    protected function expand($type)
    {
        ...
        if ($this->isTypeAnArray($type)) {
            return $this->expand(substr($type, 0, -2)) . $this->OPERATOR_ARRAY;
        }

        if ($this->isRelativeType($type) && !$this->isTypeAKeyword($type)) {
            $type_parts = explode($this->OPERATOR_NAMESPACE, $type, 2);
            ...
            if (!isset($namespace_aliases[$type_parts[0]])) {
                ...
                if ('' !== $namespace) {
                    $namespace .= $this->OPERATOR_NAMESPACE;
                }
                return $this->OPERATOR_NAMESPACE . $namespace . $type;
            }

            $type_parts[0] = $namespace_aliases[$type_parts[0]];
            $type = implode($this->OPERATOR_NAMESPACE, $type_parts);
        }
    ...
    {
        return substr($type, -2) === $this->OPERATOR_ARRAY;
    }
    ...
    {
        return ($type[0] !== $this->OPERATOR_NAMESPACE)
