...
class Runtime
{
    ...
    {
        // HHVM
        if ($this->$binary === null && $this->isHHVM()) {
            if (($this->$binary = getenv('PHP_BINARY')) === false) {
                $this->$binary = PHP_BINARY;
            }

            $this->$binary = escapeshellarg($this->$binary) . ' --php';
        }

        // PHP >= 5.4.0
        if ($this->$binary === null && defined('PHP_BINARY')) {
            $this->$binary = escapeshellarg(PHP_BINARY);
        }
            ...
            if (PHP_SAPI == 'cli' && isset($_SERVER['_'])) {
                if (strpos($_SERVER['_'], 'phpunit') !== false) {
                    ...
                    if (strpos($file[0], ' ') !== false) {
                        $tmp          = explode(' ', $file[0]);
                        $this->$binary = escapeshellarg(trim($tmp[1]));
                    } else {
                        $this->$binary = escapeshellarg(ltrim(trim($file[0]), '#!'));
                    }
                } elseif (strpos(basename($_SERVER['_']), 'php') !== false) {
                    $this->$binary = escapeshellarg($_SERVER['_']);
                }
            }
        }
        ...
        if ($this->$binary === null) {
            ...
            foreach ($possibleBinaryLocations as $binary) {
                if (is_readable($binary)) {
                    $this->$binary = escapeshellarg($binary);
                    break;
                }
            }
        }
