...
class PHPUnit_Util_Test
{
    ...
    private static function getLinesToBeCoveredOrUsed($className, $methodName, $mode)
    {
        ...
        foreach (array_unique($list) as $element) {
            ...
            $codeList = array_merge(
                $codeList,
                $this->resolveElementToReflectionObjects($element)
            );
        }

        return $this->resolveReflectionObjectsToLines($codeList);
    }
    ...
    public static function getRequirements($className, $methodName)
    {
        ...
        if ($count = preg_match_all($this->REGEX_REQUIRES_OS, $docComment, $matches)) {
            $requires['OS'] = sprintf(
                '/%s/i',
                addcslashes($matches['value'][$count - 1], '/')
            );
        }
        if ($count = preg_match_all($this->REGEX_REQUIRES_VERSION, $docComment, $matches)) {
            for ($i = 0; $i < $count; $i++) {
        ...
        if ($count = preg_match_all($this->REGEX_REQUIRES, $docComment, $matches)) {
            for ($i = 0; $i < $count; $i++) {
    ...
    public static function getExpectedException($className, $methodName)
    {
        ...
        if (preg_match($this->REGEX_EXPECTED_EXCEPTION, $docComment, $matches)) {
            $annotations = $this->parseTestMethodAnnotations(
                $className,
                $methodName
            );
            ...
            if (isset($matches[2])) {
                ...
                $message = $this->parseAnnotationContent(
                    $annotations['method']['expectedExceptionMessage'][0]
                );
            }

            if (isset($annotations['method']['expectedExceptionMessageRegExp'])) {
                $messageRegExp = $this->parseAnnotationContent(
            ...
            if (isset($matches[3])) {
                ...
                $code = $this->parseAnnotationContent(
    ...
    public static function getProvidedData($className, $methodName)
    {
        ...
        if ($dataProviderData = $this->getDataFromDataProviderAnnotation($docComment, $className, $methodName)) {
            $data = $dataProviderData;
        }

        if ($testWithData = $this->getDataFromTestWithAnnotation($docComment)) {
            $data = $testWithData;
        }
    ...
    {
        if (preg_match($this->REGEX_DATA_PROVIDER, $docComment, $matches)) {
    ...
    public static function getDataFromTestWithAnnotation($docComment)
    {
        ...
        if (preg_match($this->REGEX_TEST_WITH, $docComment, $matches, PREG_OFFSET_CAPTURE)) {
    ...
    public static function parseTestMethodAnnotations($className, $methodName = '')
    {
        if (!isset($this->$annotationCache[$className])) {
            ...
            $this->$annotationCache[$className] = $this->parseAnnotations($class->getDocComment());
        }

        if (!empty($methodName) && !isset($this->$annotationCache[$className . '::' . $methodName])) {
            try {
            ...
            $this->$annotationCache[$className . '::' . $methodName] = $annotations;
        }

        return [
          'class'  => $this->$annotationCache[$className],
          'method' => !empty($methodName) ? $this->$annotationCache[$className . '::' . $methodName] : []
        ];
    }
    ...
    {
        return [
          'backupGlobals' => $this->getBooleanAnnotationSetting(
              $className,
              $methodName,
              'backupGlobals'
          ),
          'backupStaticAttributes' => $this->getBooleanAnnotationSetting(
              $className,
              $methodName,
    ...
    {
        return $this->getBooleanAnnotationSetting(
            $className,
            $methodName,
            'errorHandler'
        );
    ...
    {
        $groups = array_flip($this->getGroups($className, $methodName));
        $size   = $this->UNKNOWN;
        $class  = new ReflectionClass($className);
    ...
    public static function getHookMethods($className)
    {
        if (!class_exists($className, false)) {
            return $this->emptyHookMethodsArray();
        }

        if (!isset($this->$hookMethods[$className])) {
            $this->$hookMethods[$className] = $this->emptyHookMethodsArray();
            ...
            try {
                ...
                foreach ($class->getMethods() as $method) {
                    if ($this->isBeforeClassMethod($method)) {
                        $this->$hookMethods[$className]['beforeClass'][] = $method->getName();
                    }

                    if ($this->isBeforeMethod($method)) {
                        $this->$hookMethods[$className]['before'][] = $method->getName();
                    }

                    if ($this->isAfterMethod($method)) {
                        $this->$hookMethods[$className]['after'][] = $method->getName();
                    }

                    if ($this->isAfterClassMethod($method)) {
                        $this->$hookMethods[$className]['afterClass'][] = $method->getName();
                    }
                }
            } catch (ReflectionException $e) {
            }
        ...
        return $this->$hookMethods[$className];
    }
