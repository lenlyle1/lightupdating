...
class Smarty_Internal_Configfileparser #line 80 "smarty_internal_configfileparser.php"
{
    ...
    public function &instance($new_instance = null)
    {
        $instance = null;
    ...
    private $escapes_single = Array('\\' => '\\',
                                           '\'' => '\'');

    private function parse_single_quoted_string($qstr)
    {
        ...
        foreach ($ss as $s) {
            if (strlen($s) === 2 && $s[0] === '\\') {
                if (isset($this->$escapes_single[$s[1]])) {
                    $s = $this->$escapes_single[$s[1]];
                }
            }
    ...
    private function parse_double_quoted_string($qstr)
    {
        $inner_str = substr($qstr, 1, strlen($qstr) - 2);
        return stripcslashes($inner_str);
    }

    private function parse_tripple_double_quoted_string($qstr)
    {
    ...
    public $yy_reduce_ofst = array(
        /*     0 */
        - 10, - 1, - 1, - 1, - 20, 10, 12, 8, 14, 7,
        /*    10 */
        - 11,
    );
    public $yyExpectedTokens = array(
        /* 0 */
        array(),
        /* 1 */
        array(5, 17, 18,),
    ...
    public function yy_destructor($yymajor, $yypminor)
    {
        switch ($yymajor) {
            default:
    ...
    public function yy_get_expected_tokens($token)
    {
        ...
        $expected = $this->$yyExpectedTokens[$state];
        if (in_array($token, $this->$yyExpectedTokens[$state], true)) {
            return $expected;
        }
        $stack = $this->yystack;
        $yyidx = $this->yyidx;
        do {
            ...
            if ($yyact >= $this->YYNSTATE && $yyact < $this->YYNSTATE + $this->YYNRULE) {
                // reduce action
                $done = 0;
                do {
                    if ($done ++ == 100) {
                    ...
                    $yyruleno = $yyact - $this->YYNSTATE;
                    $this->yyidx -= $this->$yyRuleInfo[$yyruleno]['rhs'];
                    $nextstate = $this->yy_find_reduce_action(
                        $this->yystack[$this->yyidx]->stateno,
                        $this->$yyRuleInfo[$yyruleno]['lhs']);
                    if (isset($this->$yyExpectedTokens[$nextstate])) {
                        $expected = array_merge($expected, $this->$yyExpectedTokens[$nextstate]);
                        if (in_array($token,
                                     $this->$yyExpectedTokens[$nextstate], true)) {
                            $this->yyidx = $yyidx;
                            $this->yystack = $stack;
                    ...
                    if ($nextstate < $this->YYNSTATE) {
                        // we need to shift a non-terminal
                        $this->yyidx ++;
                        $x = new TPC_yyStackEntry;
                        $x->stateno = $nextstate;
                        $x->major = $this->$yyRuleInfo[$yyruleno]['lhs'];
                        $this->yystack[$this->yyidx] = $x;
                        continue 2;
                    } elseif ($nextstate == $this->YYNSTATE + $this->YYNRULE + 1) {
                        $this->yyidx = $yyidx;
                        $this->yystack = $stack;
                        ...
                        // syntax error!
                        return array_unique($expected);
                    } elseif ($nextstate === $this->YY_NO_ACTION) {
                        $this->yyidx = $yyidx;
                        $this->yystack = $stack;
            ...
            if ($yyact >= $this->YYNSTATE && $yyact < $this->YYNSTATE + $this->YYNRULE) {
                ...
                do {
                    ...
                    $nextstate = $this->yy_find_reduce_action(
                        ...
                        $this->$yyRuleInfo[$yyruleno]['lhs']);
                    if (isset($this->$yyExpectedTokens[$nextstate]) &&
                        in_array($token, $this->$yyExpectedTokens[$nextstate], true)
                    ) {
                        $this->yyidx = $yyidx;
                        $this->yystack = $stack;
    ...
    public function yy_find_shift_action($iLookAhead)
    {
        ...
        /* if ($this->yyidx < 0) return $this->YY_NO_ACTION;  */
        if (!isset($this->$yy_shift_ofst[$stateno])) {
            // no shift actions
            return $this->$yy_default[$stateno];
        }
        $i = $this->$yy_shift_ofst[$stateno];
        if ($i === $this->YY_SHIFT_USE_DFLT) {
            return $this->$yy_default[$stateno];
        }
        if ($iLookAhead == $this->YYNOCODE) {
            return $this->YY_NO_ACTION;
        }
        $i += $iLookAhead;
        if ($i < 0 || $i >= $this->YY_SZ_ACTTAB ||
            $this->$yy_lookahead[$i] != $iLookAhead
        ) {
            if (count($this->$yyFallback) && $iLookAhead < count($this->$yyFallback)
    ...
    public function yy_find_reduce_action($stateno, $iLookAhead)
    {
        ...
        if (!isset($this->$yy_reduce_ofst[$stateno])) {
            return $this->$yy_default[$stateno];
        }
        $i = $this->$yy_reduce_ofst[$stateno];
        if ($i == $this->YY_REDUCE_USE_DFLT) {
            return $this->$yy_default[$stateno];
        }
        if ($iLookAhead == $this->YYNOCODE) {
            return $this->YY_NO_ACTION;
    ...
    function yy_r13()
    {
        $this->_retvalue = $this->parse_single_quoted_string($this->yystack[$this->yyidx + 0]->minor);
    }
    #line 734 "smarty_internal_configfileparser.php"
    #line 197 "smarty_internal_configfileparser.y"
    function yy_r14()
    {
        $this->_retvalue = $this->parse_double_quoted_string($this->yystack[$this->yyidx + 0]->minor);
    }
    #line 739 "smarty_internal_configfileparser.php"
    #line 201 "smarty_internal_configfileparser.y"
    function yy_r15()
    {
        $this->_retvalue = $this->parse_tripple_double_quoted_string($this->yystack[$this->yyidx + - 1]->minor);
    }
    ...
    public function yy_reduce($yyruleno)
    {
        ...
        if ($this->yyTraceFILE && $yyruleno >= 0
            && $yyruleno < count($this->$yyRuleName)
        ) {
        ...
        if (array_key_exists($yyruleno, $this->$yyReduceMap)) {
            // call the action
            $this->_retvalue = null;
            $this->{'yy_r' . $this->$yyReduceMap[$yyruleno]}();
            $yy_lefthand_side = $this->_retvalue;
        }
        $yygoto = $this->$yyRuleInfo[$yyruleno]['lhs'];
        $yysize = $this->$yyRuleInfo[$yyruleno]['rhs'];
        $this->yyidx -= $yysize;
        for ($i = $yysize; $i; $i --) {
        ...
        } elseif ($yyact == $this->YYNSTATE + $this->YYNRULE + 1) {
            $this->yy_accept();
        }
    }
    ...
    public function doParse($yymajor, $yytokenvalue)
    {
        ...
        do {
            ...
            if ($yymajor < $this->YYERRORSYMBOL &&
                !$this->yy_is_expected_token($yymajor)
            ) {
                // force a syntax error
                $yyact = $this->YY_ERROR_ACTION;
            }
            if ($yyact < $this->YYNSTATE) {
            ...
            } elseif ($yyact < $this->YYNSTATE + $this->YYNRULE) {
                $this->yy_reduce($yyact - $this->YYNSTATE);
                ...
                if ($this->YYERRORSYMBOL) {
                    ...
                    if ($yymx == $this->YYERRORSYMBOL || $yyerrorhit) {
                        ...
                        while ($this->yyidx >= 0 &&
                            ...
                            ($yyact = $this->yy_find_shift_action($this->YYERRORSYMBOL)) >= $this->YYNSTATE
                        ) {
                            $this->yy_pop_parser_stack();
                        }
                        if ($this->yyidx < 0 || $yymajor == 0) {
                            ...
                            $yymajor = $this->YYNOCODE;
                        } elseif ($yymx != $this->YYERRORSYMBOL) {
                            $u2 = 0;
                            $this->yy_shift($yyact, $this->YYERRORSYMBOL, $u2);
                        }
                    }
                    $this->yyerrcnt = 3;
                    $yyerrorhit = 1;
