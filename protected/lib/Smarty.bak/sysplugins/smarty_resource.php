...
abstract class Smarty_Resource
{
     ...
     */
    public function load(Smarty $smarty, $type)
    {
        // try smarty's cache
        ...
        if (isset($smarty->registered_resources[$type])) {
            ...
            if (!isset($this->$resources['registered'])) {
                $this->$resources['registered'] = new Smarty_Internal_Resource_Registered();
            }
            if (!isset($smarty->_resource_handlers[$type])) {
                $smarty->_resource_handlers[$type] = $this->$resources['registered'];
            }
        ...
        // try sysplugins dir
        if (isset($this->$sysplugins[$type])) {
            if (!isset($this->$resources[$type])) {
                $_resource_class = 'Smarty_Internal_Resource_' . ucfirst($type);
                $this->$resources[$type] = new $_resource_class();
            }

            return $smarty->_resource_handlers[$type] = $this->$resources[$type];
        }
        ...
        if (in_array($type, $_known_stream)) {
            ...
            if (!isset($this->$resources['stream'])) {
                $this->$resources['stream'] = new Smarty_Internal_Resource_Stream();
            }

            return $smarty->_resource_handlers[$type] = $this->$resources['stream'];
        }
     ...
     * @return void
     */
    protected function parseResourceName($resource_name, $default_resource, &$name, &$type)
    {
     ...
     * @return string unique resource name
     */
    public function getUniqueTemplateName($template, $template_resource)
    {
        $this->parseResourceName($template_resource, $template->smarty->default_resource_type, $name, $type);
     ...
     */
    public function source(Smarty_Internal_Template $_template = null, Smarty $smarty = null, $template_resource = null)
    {
        if ($_template) {
        ...
        $this->parseResourceName($template_resource, $smarty->default_resource_type, $name, $type);
        ...
        if (isset($this->$sources[$_cache_key])) {
            return $this->$sources[$_cache_key];
        }
        ...
        // runtime cache
        $this->$sources[$_cache_key] = $source;
     ...
     */
    public function config(Smarty_Internal_Config $_config)
    {
        $_incompatible_resources = array('eval' => true, 'string' => true, 'extends' => true, 'php' => true);
        $config_resource = $_config->config_resource;
        $smarty = $_config->smarty;

        // parse resource_name
        $this->parseResourceName($config_resource, $smarty->default_config_type, $name, $type);
