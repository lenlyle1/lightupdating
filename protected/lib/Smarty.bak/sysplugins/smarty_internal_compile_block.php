...
class Smarty_Internal_Compile_Block extends Smarty_Internal_CompileBase
{
     ...
     * @var array
     */
    public $nested_block_names = array();
    ...
    public function compile($args, $compiler)
    {
        ...
        if ($compiler->inheritance_child) {
            array_unshift($this->$nested_block_names, $_name);
            // build {block} for child block
            $this->$block_data[$_name]['source'] =
            ...
            if ($_attr['nocache']) {
                $this->$block_data[$_name]['source'] .= ' nocache';
            }
            $this->$block_data[$_name]['source'] .= $compiler->smarty->right_delimiter;
     ...
     */
    function compileChildBlock($compiler, $_name = null)
    {
        if ($compiler->inheritance_child) {
        ...
        if (strpos($compiler->template->block_data[$_name]['source'], $this->parent) !== false) {
            $_output = str_replace($this->parent, $compiler->parser->current_buffer->to_smarty_php(), $_tpl->compiler->compileTemplate($_tpl, $nocache));
     ...
     * @return string   compiled code of child block
     */
    function compileParentBlock($compiler, $_name = null)
    {
     ...
     *
     */
    function blockSource($compiler, $source)
    {
